...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, args):
    ...
    for iteration in range(first_iter, opt.iterations + 1):        
        ...
        if opt.include_feature:
            ...
            if look_test and iteration % 20 == 0:
                print(viewpoint_cam.image_name)
                feature_look = gt_language_feature.permute(1,2,0).detach().cpu().numpy()
                img = cv2.cvtColor(feature_look, cv2.COLOR_RGB2BGR)
                cv2.imwrite('./look.jpg', img*255)

                feature_look = image.permute(1,2,0).detach().cpu().numpy()
                img = cv2.cvtColor(feature_look, cv2.COLOR_RGB2BGR)
            ...
            if look_test and iteration % 20 == 0:
                ...
                feature_look = gt_image.permute(1,2,0).detach().cpu().numpy()
                img = cv2.cvtColor(feature_look, cv2.COLOR_RGB2BGR)
                cv2.imwrite('./look_img.jpg', img*255)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--checkpoint_iterations", nargs="+", type=int, default=[500, 1000])
